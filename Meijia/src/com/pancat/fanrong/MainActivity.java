...
public class MainActivity extends ActivityGroup implements OnClickListener {
	...
	private static ImageView cancle_location;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		// 初始化消息
		mNotification = new Notification(R.drawable.aif, "正在上传中...",
	...
	private void showpopwindow() {
		...
		Button sendmoment = (Button) dropDownList
				.findViewById(R.id.sendmomentbtn);
		Button usersetting = (Button) dropDownList
				.findViewById(R.id.usersettingbtn);
		...
		usersetting.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				if (AuthorizeMgr.getInstance().hasLogined() == false) {
					...
					signInIntent.setClass(MainActivity.this,
	...
	public static Handler handler2 = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			case 1:
				...
				cancle_location.setVisibility(View.VISIBLE);
				break;
			default:
				break;
			}
	...
	private void openAddDialog() {
		...
		addPicDialog.setView(getLayoutInflater().inflate(
				R.layout.dialog_add_circle, null));
		addPicDialog.show();
		...
		LinearLayout uploadCamera = (LinearLayout) addPicWindow
				.findViewById(R.id.upload_camera);
		LinearLayout uploadFile = (LinearLayout) addPicWindow
				.findViewById(R.id.upload_file);
		final TextView circleDescription = (TextView) addPicWindow
				.findViewById(R.id.circle_description);
		CircleImageView userimage = (CircleImageView) addPicWindow
				.findViewById(R.id.user_img);
		// 获取用户头像文件
		File userimagefile = new File(Constants.USER_IMAGE_PATH);
		if (userimagefile.exists()) {
			Bitmap imageBitmap = BitmapFactory.decodeFile(userimagefile
					.getPath());
			userimage.setImageBitmap(imageBitmap);
		}
         cancle_location = (ImageView) addPicWindow
				.findViewById(R.id.cancle_location);

		btnSendCircle = (ImageButton) addPicWindow
				.findViewById(R.id.btn_send_circle);
		btnAddLocation = (Button) addPicWindow
				.findViewById(R.id.btn_add_location);
		selectedImgGridView = (GridView) addPicWindow
				.findViewById(R.id.selected_image_gridview);
		selectedImgGridView.setSelector(new ColorDrawable(Color.TRANSPARENT));
		selectedImgAdapter = new SelectedImgAdapter(this, getResources(),
				handler);
		...
		ImageView locimage=(ImageView)addPicWindow.findViewById(R.id.loc_img);
		...
		OnClickListener toLocation=new OnClickListener() {
		...
		//取消位置
		cancle_location.setOnClickListener(new OnClickListener() {

			@Override
			public void onClick(View arg0) {
				// TODO Auto-generated method stub
				btnAddLocation.setText(R.string.circle_add_location);
				cancle_location.setVisibility(View.GONE);
			}
		...
		uploadCamera.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				File filedir = new File(BASE_FILE_PATH);
				if (!filedir.exists()) {
					filedir.mkdir();
				}
				File file = new File(BASE_FILE_PATH, String.valueOf(System
			...
			public void onClick(View v) {
				...
				if (pathList.size() > 0) {
					...
					mNotification.contentView.setTextViewText(
							R.id.down_tv,
							"上传中..." + finishedUploadNum + " / "
									+ pathList.size());
					showNotification();
					addPicDialog.dismiss();
					Toast.makeText(MainActivity.this, "后台上传中...请稍后",
							Toast.LENGTH_LONG).show();
					RestClient.getInstance().postFromAbsoluteUrl(
							MainActivity.this, requestUrl, params,
								...
								public void onFailure(Throwable error,
									...
									mNotification.contentView
											.setViewVisibility(R.id.pb,
													View.GONE);
									mNotification.contentView
											.setImageViewResource(R.id.tip_img,
													R.drawable.apy);
									mNotification.contentView.setTextViewText(
											R.id.down_tv, "上传失败!!");
									showNotification();
		...
		RestClient.getInstance().postFromAbsoluteUrl(this, url, params,
				new AsyncHttpResponseHandler() {
					...
					public void onFailure(Throwable error, String content) {
						super.onFailure(error, content);
						mNotification.contentView.setViewVisibility(R.id.pb,
								View.GONE);
						mNotification.contentView.setImageViewResource(
								R.id.tip_img, R.drawable.apy);
						mNotification.contentView
								.setTextViewText(R.id.down_tv, "上传中断，上传成功"
										+ finishedUploadNum + "，上传失败"
										+ (pathList.size() - finishedUploadNum));
						showNotification();
					}
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (resultCode == Activity.RESULT_OK) {
			if (requestCode == FROM_CAMERA) {
				// 图片从相机获取
				Bitmap bitmap = BitmapFactory.decodeFile(cameraPicPath,
						getBitmapOptions(5));
				Bitmap resizeBitmap = ImageResizer.decodeSampledBitmapFromFile(
						cameraPicPath, 480, 800);
				...
				ArrayList<String> list = (ArrayList<String>) data
						.getSerializableExtra(ImageGridActivity.SELECTED_IMAGE_LIST);
				Log.i(ImageGridActivity.SELECTED_IMAGE_LIST,
						String.valueOf(list));
			}
		}
	}
	...
	public void onClick(View v) {
		...
		case R.id.tab_home:
			resetBtn();
			ImageButton btnTabHome = (ImageButton) tabHome
		...
		case R.id.tab_order:
			resetBtn();
			ImageButton btnTabOrder = (ImageButton) tabOrder
					.findViewById(R.id.btn_tab_order);
		...
		case R.id.tab_me:
			...
			resetBtn();
			ImageButton btnTabMe = (ImageButton) tabMe
					.findViewById(R.id.btn_tab_me);
		...
		case R.id.tab_moment:
			resetBtn();
			ImageButton btnTabMoment = (ImageButton) tabMoment
					.findViewById(R.id.btn_tab_moment);
	...
	private void initMsgPush() {
		...
		try {
			if (msgPushType == ConfigHelperUtils.GETUIMSGPUSH) {
				...
				appsecret = ConfigHelperUtils.getMetaValue(this,
						"PUSH_APPSECRET");
				appkey = ConfigHelperUtils.getMetaValue(this, "PUSH_APPKEY");
				Log.d(TAG, "appid:" + appid + " appsecret:" + appsecret
						+ " appkey:" + appkey);
		...
		if (msgPushType == ConfigHelperUtils.GETUIMSGPUSH) {
			...
			temp.setBaiDuNotificationLayoutId(
					R.layout.notification_custom_builder,
					R.id.notification_icon, R.id.notification_title,
					R.id.notification_text, android.R.drawable.ic_media_play);
